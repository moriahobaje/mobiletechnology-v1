  ...
  search($event) {
    ...
    this.toDoItems.forEach(toDoItem => {
      ...
      keywords.forEach(keyword => {
        ...
        /*if (toDoItem.hasOwnProperty(keyword)) {
          return;
        }

        const toDoItemStr = JSON.stringify(toDoItem).toLocaleLowerCase();

        if (toDoItemStr.includes(keyword)) {
          this.searchResults.push(toDoItem);
        }*/

        /*if (
          toDoItem.title.toLocaleLowerCase().includes(keyword) ||
          toDoItem.content.toLocaleLowerCase().includes(keyword)
          ) {
            this.searchResults.push(toDoItem);
          }*/
      });
